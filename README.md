...
**EDA를 통한 가설 도출**,  
**운영 DB 기준 SQL KPI 정의·검증 및 Python 재현**,  
**머신러닝 모델링을 통한 이탈 고객 예측**,  
그리고 **Streamlit 기반 자동화 리포트 및 대시보드 구축**까지 포함하는  
**실무 환경을 가정한 End-to-End 데이터 분석 프로젝트**입니다.  
...
### 1. KPI 기반 이탈 구조
- Month-to-month 계약 + 초기 고객(0–5개월)에서 이탈 집중
- Fiber optic 서비스 고객이 주요 고위험군  
👉 “계약 구조 + 초기 경험”이 이탈의 핵심 원인  
  
### 2. 세그먼트 단위에서 이탈 발생
- Month-to-month × Fiber optic × 초기 고객  
  → 최고 이탈률  
👉 단일 변수보다 **조합 기반 분석이 중요**  
  
### 3. 요금은 보조 요인
- 초기 고객에서는 요금 민감도 존재
- 하지만 구조(계약/서비스)가 더 큰 영향
  
### 4. 모델링 전략
- Recall 중심으로 Threshold 조정
- 이탈 고객 탐지율 약 93%
  
### 5. 실행 가능한 전략
- 초기 고객 온보딩 강화
- 장기 계약 전환 유도
- Fiber optic 고객 경험 개선  
...
### ⭐ 핵심 역량  
- Python 기반 데이터 분석 경험
- EDA → KPI → 모델링 → 서비스화까지 이어지는 End-to-End 분석 설계
- SQL 기반 KPI 정의 및 데이터 정합성 검증 경험
- 머신러닝 모델 설계 및 성능 개선 (Recall 최적화)
- Recall 중심 모델링 및 Threshold 최적화 경험
